 ...
 function updateLastLogIn($user_id)
  ...
  //    }
     ...
     function page_require_level($require_level) {
      ...
      // If group status is inactive (banned)
      elseif ($login_level['group_status'] === '0') {
          $session->msg('d', 'This level user has been banned!');
          redirect('home.php', false);
      }
      // Check if current user level meets the required level
      elseif ($current_user['user_level'] <= (int)$require_level) {
          return true;
      }
      elseif ($current_user['user_level'] >= (int)$require_level) {
  ...
  //     // Ensure current_user() is returning a valid user
  //     if (!$current_user) {
  ...
  //     // Check if group status is inactive
  ...
  //     // Admin (group 1) should have access to all levels
  //     // Special (group 2) can access level 1 and 2
  //     // User (group 3) can only access level 3
  //     if ($current_user['user_level'] == 1) { // Admin
  //         return true;
  //     } elseif ($current_user['user_level'] == 2 && $require_level <= 2) { // Special
  //         return true;
  //     } elseif ($current_user['user_level'] == 3 && $require_level == 3) { // User
  //         return true;
  //     } else {
  //         $session->msg('d', 'Sorry! You don\'t have permission to view this page.');
  //         redirect('home.php', false);
...
function find_product_by_id($product_id) {
  ...
  $result = $db->query($sql);
  return ($db->num_rows($result) === 1) ? $db->fetch_assoc($result) : null;
}
